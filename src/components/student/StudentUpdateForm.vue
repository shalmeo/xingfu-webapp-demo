<template>
  <default-form>
    ...
    <div class="form-outline mb-2">
      ...
      <default-input
        type="date"
        v-model:value="student.access_start"
        required
      />
    </div>
    <div class="form-outline mb-2">
      <default-label>Запись пассивна с:</default-label>
      <default-input type="date" v-model:value="student.access_end" required />
    </div>
  </default-form>
</template>
...
function setStudent(getStudent) {
  student.surname = getStudent.surname;
  student.name = getStudent.name;
  student.patronymic = getStudent.patronymic;
  student.phone = getStudent.phone;
  student.telegram_id = getStudent.telegram_id;
  student.telegram_username = getStudent.telegram_username;
  student.birthday = getStudent.birthday;
  student.email = getStudent.email;
  student.timezone = getStudent.timezone;
  student.access_start = getStudent.access_start;
  student.access_end = getStudent.access_end;
}
...
onMounted(async () => {
  ...
  const response = await sendGetRequest(http, `/root/student/${studentId}`)
  setStudent(response.data)

  initWebApp("Изменить запись", () =>
    sendUpdateRequest(http, `/root/student/${studentId}`, student)
  );
});
</script>
